...
export default function CreateOrder({ onSave }) {
  ...
  const validatePhone = (value) => {
    ...
    if (!cleaned || cleaned.length !== 10) {
      return "Please enter a valid 10-digit phone number (e.g. 0540877214)";
    }
    return '';
  };
  ...
  const handleSubmit = () => {
    ...
    const error1 = validatePhone(cleanedPhone);

    let error2 = '';
    const cleanedPhone2 = phone2.replace(/\D/g, '');
    if (showPhone2 && cleanedPhone2) {
      error2 = validatePhone(cleanedPhone2);
    }

    if (error1 || error2) {
      setPhone(cleanedPhone);
      setPhoneError(error1);
      if (showPhone2) {
        setPhone2(cleanedPhone2);
        setPhone2Error(error2);
      }
      return;
    }
    ...
    const newOrder = {
      ...
      phone: cleanedPhone,
      phone2: (showPhone2 && cleanedPhone2) ? cleanedPhone2 : null,
      wilaya,
              ...
              <>
                <div className="flex flex-col md:flex-row bg-[#112C70]/80 rounded border border-gray-600 overflow-hidden relative mt-4">
                  ...
                  <input
                    ...
                    className="w-full md:w-2/3 p-2 bg-transparent text-white outline-none"
                  />
                  <button onClick={() => { setShowPhone2(false); setPhone2(''); setPhone2Error(''); }} className="absolute right-2 top-2 text-gray-500 hover:text-red-400 bg-[#112C70]/80 p-1 md:p-0 rounded-full md:bg-transparent md:rounded-none">
                    <X size={20} />
                  </button>
                </div>
                {phone2Error && (
                  <p className="text-red-400 text-sm mt-1 ml-1">{phone2Error}</p>
                )}
              </>
            )}
